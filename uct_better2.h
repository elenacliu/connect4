...
public:
	...
	// isExpandable目前不必修改
	bool isExpandable() {
		if (isTerminal())	// 如果是终态，肯定false；被删除了孩子节点的节点必定不是终态，因此利用expandableNum进行判断
			return false;
	...
	Node* expand() {	// expand是增加孩子节点，减少可扩展节点
		...
		bool my_win = (3 - chessman == USER) ? userWin(x, y, row, col, newBoard) : machineWin(x, y, row, col, newBoard);
		// 如果我方获胜，则删除其余所有的兄弟节点，并且将可扩展数置为1
		if (my_win) {
			for (int i = 0; i < y; i++) {
				if (children[i])
					delete children[i];
			...
			expandableNum = 0;
			// TODO: check the logic of expandCol[]
		}
		else {
...
class UCT {
	...
	// 判断对方的局面
	vector<pair<int, int>> checkYOU(int* top, int** board, int you) {
		...
		for (int i = 0; i < col; i++) {
			if (top[i] != 0) {
				...
				flag = (you == USER) ? userWin(top[i] - 1, i, row, col, board) : machineWin(top[i] - 1, i, row, col, board);
				if (flag)
					ans.push_back({ top[i] - 1, i });
	...
	pair<int, int> checkME(int* top, int** board, int me) {
		pair<int, int> ans = { -1, -1 };
		bool flag;
		for (int i = 0; i < col; i++) {
			if (top[i] != 0) {
				board[top[i] - 1][i] = me;	// place a chess
				flag = (me == USER) ? userWin(top[i] - 1, i, row, col, board) : machineWin(top[i] - 1, i, row, col, board);
				if (flag) {
					ans = { top[i] - 1,i };
					board[top[i] - 1][i] = 0;
					break;
	...
	int defaultPolicy(Node* v) {		// 根据v当前状态进行判定
		...
		int tmp_chessman = v->getChessman();	// tmp_chessman是对方
		...
		while (p == UNTERMINAL) {
			// 先判断自己有无制胜位
			ans_pair = checkME(tmp_top, tmp_board, 3 - tmp_chessman);
			// 自己有制胜位
			if (ans_pair.first != -1) {
				p = ((3 - tmp_chessman) == USER) ? USER_WIN : MACHINE_WIN;
			}
			// 自己没有制胜位
			else {
				// 判断别人有无制胜位
				ans_vec = checkYOU(tmp_top, tmp_board, tmp_chessman);
				int length = ans_vec.size();
				if (length >= 2)
					p = (tmp_chessman == USER) ? USER_WIN : MACHINE_WIN;
				else if (length == 1) {
					x = ans_vec[0].first;
					y = ans_vec[0].second;
